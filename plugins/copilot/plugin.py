...
class CopilotPlugin(BasePlugin):
    name = "copilot"
    description = "Reacts to completion pings or JSONL with activity field"
    version = "0.1.0"
    events_jsonl_basename = "copilot_events.jsonl"
    ...
    def _event_path(self) -> Path:
        ...
        if path is None:
            raise RuntimeError("CopilotPlugin.events_jsonl_basename must be set")
        return path
...
def build_hook_event(args: list[str]) -> Optional[Tuple[Path, dict[str, object]]]:
    if len(args) < 2 or args[0].lower() != "copilot" or args[1].lower() != "completion":
        return None
    path = CopilotPlugin.events_jsonl_path()
    if path is None:
        return None
    return (path, {"ts": time.time(), "activity": CodingActivity.EXPLORING.value})
