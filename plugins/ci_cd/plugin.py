...
class CiCdPlugin(BasePlugin):
    name = "ci_cd"
    description = "Reacts to CI/CD pass or fail hooks"
    version = "0.1.0"
    events_jsonl_basename = "ci_events.jsonl"
    ...
    def _event_path(self) -> Path:
        ...
        if path is None:
            raise RuntimeError("CiCdPlugin.events_jsonl_basename must be set")
        return path
...
def build_hook_event(args: list[str]) -> Optional[Tuple[Path, dict[str, object]]]:
    if len(args) < 2 or args[0].lower() != "ci":
        return None
    sub = args[1].lower()
    if sub not in ("pass", "fail"):
        return None
    activity = CodingActivity.TESTS_PASSED if sub == "pass" else CodingActivity.TESTS_FAILED
    path = CiCdPlugin.events_jsonl_path()
    if path is None:
        return None
